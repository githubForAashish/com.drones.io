...
public interface IDronesRepository extends CrudRepository<Drone, UUID> {
  ...
  @Query(
      "SELECT d FROM Drone d LEFT OUTER JOIN d.medications m WHERE d.batteryRemaining >= :batteryThreshold AND (d.state = 'IDLE' OR d.state = 'LOADING') GROUP BY d HAVING COALESCE(SUM(m.weight), 0) < d.weightLimit")
