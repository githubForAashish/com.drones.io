...
public class DronesService {
    ...
    public DroneDTO registerDrone(RegisterDroneRequestSchema drone) {
        ...
        if (dronesRepository.count() >= FLEET_CAPACITY) throw ApplicationException.create(FLEET_CAPACITY_REACHED);
    ...
    public List<MedicationDTO> getLoadedMedications(String droneSerialNumber) {
        ...
        return drone.getMedications().stream().map(medication -> modelMapper.map(medication, MedicationDTO.class)).collect(Collectors.toList());
    }

    public DroneDTO getDroneBySerialNumber(String droneSerialNumber) {
        return modelMapper.map(dronesRepository.findBySerialNumber(droneSerialNumber).orElseThrow(() -> ApplicationException.create(DRONE_NOT_FOUND)), DroneDTO.class);
    }
    ...
    public DroneDTO loadMedications(String droneSerialNumber, List<MedicationDTO> medications) {
        ...
        List<Medication> newMedications = medications.stream().map(medicationDTO -> modelMapper.map(medicationDTO, Medication.class)).collect(Collectors.toList());
        ...
        if (drone.getBatteryRemaining() <= BATTERY_THRESHOLD) throw ApplicationException.create(DRONE_BATTERY_TOO_LOW);
    ...
    public List<DroneDTO> getDronesAvailableForLoading() {
        return dronesRepository.listDronesAvailableForLoading(BATTERY_THRESHOLD).stream().map(drone -> modelMapper.map(drone, DroneDTO.class)).collect(Collectors.toList());
    }
